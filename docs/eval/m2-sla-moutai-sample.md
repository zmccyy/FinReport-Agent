...
> 生成时间：2026-07-21T04:53:35+00:00
...
| 指标 | 阈值 | 实测 | 结果 |
|---|---|---|---|
| PARSE + EXTRACT | ≤ 180 s | 15.68 s | ✅ 通过 |

## 各阶段 SLA（spec §12.1）

| 阶段 | 阈值 | 实测 | 结果 | 备注 |
|---|---|---|---|---|
| parse | ≤ 90 s | 15.68 s | ✅ 通过 | page_count=143 |
| extract | ≤ 60 s | 0.00 s | ✅ 通过 | mock LLM (no GPU); real timing requires ai-service |
| total | ≤ 240 s | 15.68 s | ✅ 通过 | 累计耗时 |

## 总结

- M2.12 PARSE+EXTRACT：15.68 s / 180 s — ✅ 通过
- Spec §12.1 strict 检查：启用 — ✅ 通过
